...
COPY --from=cache --chown=node:node /opt/probable-futures/cache/yarn.lock .
COPY --from=cache --chown=node:node /opt/probable-futures/cache/package.json .
COPY --from=cache --chown=node:node /opt/probable-futures/cache/packages packages

RUN yarn --cwd packages/worker build:production
...
COPY --from=cache --chown=node:node /opt/probable-futures/cache/packages/worker/node_modules packages/worker/node_modules
...
CMD ["node", "/opt/probable-futures/app/packages/worker/dist/main.js"]
