...
export const exportMapToHTML = (options: Props) => {
  return `
  <!DOCTYPE html>
  <html>
    <head>
    ...
    <body>
      <div id="map" style="display:${options.compare?.show ? "none" : "block"};"></div>
      <div id="comparison-container" style="display:${
        options.compare?.show ? "block" : "none"
      };" ><div id="beforeMap"></div><div id="afterMap"></div></div>
      <script>
        (function() {
          ...
          const showComparisonMaps = ${JSON.stringify(options.compare?.show)};
          let map, beforeMap, afterMap;
          let longitude=null, latitude=null, popup=null, mapClicked, featuresBefore=[], featuresAfter=[], features=[];
          let { dataKeyBefore, dataKeyAfter,dataLayerPaintPropertiesBefore, dataLayerPaintPropertiesAfter } = ${JSON.stringify(
            options.compare || {},
          )};
